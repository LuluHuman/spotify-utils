...
interface Top {
	data: {
		me: {
			profile: { topArtists?: topArtists; topTracks?: topTracks };
		};
	};
	extensions: object;
}
...
export default function Top() {
	...
	const [top, setTop] = useState<{ topArtists?: topArtists; topTracks?: topTracks }>();
	const [moreArtistShown, setMoreArtistShown] = useState<boolean>(false);
	const [moreTracksShown, setMoreTracksShown] = useState<boolean>(false);
	const [timeRange, setTimeRange] = useState<{
		artist: "SHORT_TERM" | "MID_TERM" | "LONG_TERM";
		track: "SHORT_TERM" | "MID_TERM" | "LONG_TERM";
	}>({ artist: "SHORT_TERM", track: "SHORT_TERM" });

	const [artistDensity, setArtistDensity] = useState<number>(100);

	useEffect(() => {
		function calcArtistDensity() {
			let density = Math.floor(document.body.clientWidth / 168);
			if (density < 3) density = 3;
			setArtistDensity(density);
		}
		calcArtistDensity();
		window.addEventListener("resize", calcArtistDensity);

		return () => window.removeEventListener("resize", calcArtistDensity);
	}, []);

	useEffect(() => {
		SpotifyClient?.addReadyListener(async () => {
			if (SpotifyClient?.session.isAnonymous) return (location.href = "/login");
			const top = (await SpotifyClient?.getTop(
				{
					offset: 0,
					limit: 100,
					sortBy: "AFFINITY",
					timeRange: timeRange.artist,
				},
				{
					offset: 0,
					limit: 100,
					sortBy: "AFFINITY",
					timeRange: timeRange.track,
				}
			)) as Top;
			setTop(top.data.me.profile);
		});
		...
		<div className="px-5">
					...
					<div>
						...
						<select
							onInput={(e) => {
								...
								if (!["SHORT_TERM", "MID_TERM", "LONG_TERM"].includes(value))
									return;
								setTimeRange((timeRange) => {
									const val = {
										artist: value,
										track: timeRange.track,
									} as {
										artist: "SHORT_TERM" | "MID_TERM" | "LONG_TERM";
										track: "SHORT_TERM" | "MID_TERM" | "LONG_TERM";
									};
							...
							<option value="MID_TERM">Last 6 Months</option>
							<option value="LONG_TERM">Last 12 Months</option>
						</select>
					</div>
					<button
						className="font-bold cursor-pointer"
						onClick={() => setMoreArtistShown((b) => !b)}>
						{moreArtistShown ? "Show less" : "Show more"}
					</button>
				</div>

				<ol
					className={`flex w-full overflow-hidden gap-8 justify-evenly flex-wrap ${
						moreArtistShown ? "" : "h-60"
					}`}>
					{top &&
						(moreArtistShown
							? top.topArtists?.items
							: top.topArtists?.items?.toSpliced(artistDensity || 1)
						)?.map((item, i: number) => {
							const img = item.data.visuals.avatarImage?.sources[0];
							return (
								<li
									key={i}
									className="max-w-1/4 w-40">
									{img ? (
										<Image
											...
											width={170}
											src={img?.url}
											alt={item.data.profile.name + "'s Image"}
										/>
									) : (
										<div className="rounded-full w-full aspect-square flex items-center justify-center bg-neutral-600">
											<svg
												data-encore-id="icon"
												role="img"
												aria-hidden="true"
												className="size-1/4 fill-white"
												viewBox="0 0 24 24">
									...
									<span className="w-full wrap-anywhere line-clamp-2 text-center">
										<span>
											<strong className="pr-1">{i + 1}.</strong>
			...
			<section>
				<div className="flex justify-between p-2">
					...
					<button
						className="font-bold cursor-pointer"
						onClick={() => setMoreTracksShown((b) => !b)}>
						{moreTracksShown ? "Show less" : "Show more"}
					</button>
				</div>
				<ol
					className={`flex flex-col w-full overflow-hidden gap-8 ${
						moreTracksShown ? "flex-wrap" : ""
					}`}>
					{top &&
						(moreTracksShown
							? top.topTracks?.items
							: top.topTracks?.items?.toSpliced(4)
						)?.map((item, i: number) => {
							const img = item.data.albumOfTrack.coverArt.sources[1];
							return (
								<li
									key={i}
									className="flex ml-8  justify-between">
									<div className="flex">
										<div className="flex justify-center items-center p-2 absolute left-4  size-12">
											<span>{i + 1}</span>
										</div>
										<Image
											className="rounded-sm size-12 mx-2"
											unoptimized={true}
											height={170}
											width={170}
											src={img.url}
											alt={item.data.name + "'s Album Cover"}
										/>
										<span>
